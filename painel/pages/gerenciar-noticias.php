<?php
    if(isset($_GET['excluir'])){
        ...
        $selectIamgem = Mysql::conectar()->prepare("SELECT capa FROM `tb_site.noticias` WHERE id = ?");
        $selectIamgem->execute(array($_GET['excluir']));

        $imagem = $selectIamgem->fetch()['capa'];
        ...
        <?php 
            ...
            for ($i=0; $i < $totalPaginas; $i++) { 
                ...
                if($numero == $paginaAtual){
                    echo'<a class="page-selected" href="'.INCLUDE_PATH_PAINEL.'gerenciar-noticias?pagina='.$numero.'">'.$numero.'</a>';
                }else{
                    echo'<a href="'.INCLUDE_PATH_PAINEL.'gerenciar-noticias?pagina='.$numero.'">'.$numero.'</a>';
                }
            }
        ?>
    </div><!--paginacao-->
